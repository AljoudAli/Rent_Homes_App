...
@override
  ...
  Widget build(BuildContext context) {
   return BlocBuilder<FetchDataBloc, FetchDataState>(builder: (context, state) {
      ...
      else if(state is LoadingDataSuccessfulState)
      {
        return GoogleMap(
          initialCameraPosition: CameraPosition(
              target: LatLng(
                  state.places[0].geo.lat,
                  state.places[0].geo.lng
              ),
              zoom: 9
          ),
  ...
  void setMapList() {
    for(int i=1;i<widget.places.length;i++)
    {
      myMarkers.add(Marker(
          position:LatLng(
              widget.places[i].geo.lat,
              widget.places[i].geo.lng
          ),
          markerId: MarkerId(
              LatLng(
                  widget.places[i].geo.lat,
